...
public class WeaverSpiderEntity extends SpiderEntity implements IAnimatable {

    private static final TrackedData<Boolean> PLAYING_ATTACK_ANIMATION = DataTracker.registerData(WeaverSpiderEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    ...
    protected void initGoals() {
        ...
        this.goalSelector.add(4, new WeaverSpiderAttackGoal(this, 1.5f, true));
    ...
    private <E extends IAnimatable> PlayState playStatePredicateForMove(AnimationEvent<E> event) {
        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("weaver.walk", true));
        } else if (!isPlayingAttackAnimation()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("weaver.idle", true));
        }
    ...
    private <E extends IAnimatable> PlayState playStatePredicateForAttack(AnimationEvent<E> event) {
        if (isPlayingAttackAnimation()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("weaver.attack", true));
            return PlayState.CONTINUE;
        }
    ...
    public void registerControllers(AnimationData animationData) {
        animationData.addAnimationController(new AnimationController<>(this, "moveController", 0, this::playStatePredicateForMove));
    ...
    private static class WeaverSpiderAttackGoal extends Goal {
        protected final WeaverSpiderEntity mob;
        ...
        public WeaverSpiderAttackGoal(WeaverSpiderEntity mob, double speed, boolean pauseWhenMobIdle) {
            this.mob = mob;
            this.speed = speed;
